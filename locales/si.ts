const TRANSLATIONS = {
  welcome: "Shopbook-යට තුළ ඔබව තුළ ආයුබෝවන්",
  create_business_first: "මුල්ම ව්‍යාපාර ගිණුමක් තනමු",
  business_name: "ව්‍යාපාර නම",
  invoice_feature:
    "Professional ඉන්වොයිසි පහසුවෙන් හදන්න. ගෙවීම් manage කරලා ණය මතක් කිරීම් යවා 3 ගුණයක් වේගයෙන් ණය එකතු කරගන්න.",
  hero_simplify: "ඔබේ ව්‍යාපාර invoice ",
  hero_business_invoicing: "පහසුවෙන් සාදා නියමිත වේලාවට ",
  hero_get_paid_on_time: "මුදල් ලබා ගන්න. ",
  "&": "",
  features: {
    credit_cash_invoices_title: "ණය සහ මුදල් ඉන්වොයිසි පහසුයි",
    credit_cash_invoices_description:
      "ඔබගේ සියලුම ණය සහ මුදල් විකුණුම් ඉන්වොයිසි එක තැනකින් කළමනාකරණය කරන්න.",
    invoice_history_title: "ඔබගේ සම්පූර්ණ ඉන්වොයිසි ඉතිහාසය වෙත ප්‍රවේශ වන්න",
    invoice_history_description:
      "ඕනෑම වේලාවක සියලුම පැරණි ඉන්වොයිසි බලන්න සහ සොයන්න.",
    faster_payments_title: "ස්වයංක්‍රීය මතක් කිරීම් සමඟ වේගවත් ගෙවීම්",
    faster_payments_description:
      "වේගවත් ගෙවීම් එකතු කිරීම සඳහා SMS/WhatsApp හරහා පාරිභෝගිකයින්ට ස්වයංක්‍රීයව මතක් කරන්න.",
    payment_tracking_title: "පහසු ගෙවීම් ලුහුබැඳීම",
    payment_tracking_description:
      "ලැබුණු ගෙවීම් ඉක්මනින් සටහන් කර ඔබගේ ගිණුම් ක්ෂණිකව යාවත්කාලීන කරන්න.",
    generate_reports_title: "වාර්තා සාදා සහ බෙදාගන්න",
    generate_reports_description:
      "වෘත්තීය විකුණුම් සහ ගෙවීම් වාර්තා පහසුවෙන් සාදන්න.",
    available_languages_title: "භාෂා 3කින් ලබා ගත හැක",
    available_languages_description:
      "Shopbook සිංහල, දෙමළ හෝ ඉංග්‍රීසි භාෂාවෙන් භාවිතා කරන්න.",
    achieve_goals_title: "ඔබේ ව්‍යාපාර ඉලක්ක Shopbook සමඟ ළඟා කරගන්න.",
    simplified_tagline: "ඉන්වොයිසි සිට ගෙවීම් දක්වා - ඔබ වෙනුවෙන් සරල කර ඇත",
    see_reminders_title: "ShopBook Auto ගෙවීම් මතක් කිරීම් යවා නොගෙවු ණය ඉක්මනින් එකතු කරගන්නේ  කෙසේදැයි බලන්න.",
    see_reminders_description:
      "ShopBook විසින් වෘත්තීය ගෙවීම් මතක් කිරීම් පහසුවෙන් යවන්නේ කෙසේදැයි බලන්න, එමඟින් ඔබට වේගයෙන් එකතු කිරීමට සහ නොගෙවූ ණය අඩු කිරීමට උපකාරී වේ.",
    watch_demo_video: "ආදර්ශන වීඩියෝව බලන්න",
  },

  howItWorks: {
    title: "Shopbook ක්‍රියා කරන ආකාරය",
    subtitle: "සරල පියවර 4කින් ඔබේ ඉන්වොයිසි කළමනාකරණය ආරම්භ කරන්න",
    step1_title: "ඉන්වොයිසි සාදන්න",
    step1_description:
      "මුදල් හෝ ණය විකුණුම් සඳහා ගෙවීම්  නියමිත දින (due date) සමඟ ඉන්වොයිසි සාදන්න.",
    step2_title: "WhatsApp හෝ SMS හරහා යවන්න",
    step2_description:
      "WhatsApp හෝ SMS හරහා ඔබේ පාරිභෝගිකයින්ට ක්ෂණිකව ඉන්වොයිසි ලබා දෙන්න.",
    step3_title: "ස්වයංක්‍රීය මතක් කිරීම් සකසන්න",
    step3_description:
      "ස්වයංක්‍රීය, ආචාරශීලී මතක් කිරීම් සමඟින් නියමිත වේලාවට ගෙවීම් ලැබීම සහතික කරන්න.",
    step4_title: "වාර්තා generate කරන්න",
    step4_description:
      "ඕනෑම වේලාවක සවිස්තරාත්මක වාර්තා සහ ගෙවීම් සාරාංශ බලාගන්න",
  },
  comparison: {
    title: "ව්‍යාපාර දහස් ගණනක් Shopbook වෙත මාරු වී ඇත්තේ මන්දැයි බලන්න",
    manual_process_heading: "අතින් කරන ක්‍රියාවලිය",
    manual_process_item1: "වැඩි කාලයක් ගත කරමින් invoice සෑදීම",
    manual_process_item2: "අද ගෙවීම් නියමිත අය අමතක වීම",
    manual_process_item3: "ඉන්වොයිසි නැති වීම් සහ අක්‍රමික වාර්තා තබා ගැනීම්",
    manual_process_item4: "ගෙවීම් ප්‍රමාද වීමෙන් මුදල් ප්‍රවාහ ගැටලු ඇතිවීම",
    shopbook_heading: "Shopbook සමඟ",
    shopbook_item1: "තත්පර කිහිපයකින් ඉන්වොයිසි සාදන්න",
    shopbook_item2: "WhatsApp/SMS හරහා ස්වයංක්‍රීය ගෙවීම් මතක් කිරීම්",
    shopbook_item3: "සම්පූර්ණ ඉන්වොයිසි ඉතිහාසය සහ සවිස්තරාත්මක වාර්තා",
    shopbook_item4: "3x දක්වා වේගයෙන් මුදල් එකතු කරගන්න.",
  },
  testimonialsSection: {
    title: "අපගේ usersලා පවසන දේ",
  },
  cta: {
    title: "ආරම්භ කිරීමට සූදානම්ද?",
    description:
      "දැන්ම Shopbook බාගත කර, ඔබ ඉගෙන ගත් දේ ක්‍රියාත්මක කර ඉක්මනින් ගෙවීම් ලබා ගන්න.",
  },
  faqSection: {
    main_title: "නිතර අසන ප්‍රශ්න",
    q1_question: "Shopbook සමඟ මගේ දත්ත ආරක්ෂිතද?",
    q1_answer:
      "සියලුම තොරතුරු සංකේතනය කර ආරක්ෂිතව ගබඩා කර ඇති අතර, අපි කිසිවිටෙකත් ඔබේ ව්‍යාපාර දත්ත තෙවන පාර්ශවයන් සමඟ බෙදා නොගනිමු. ඔබගේ ඉන්වොයිසි සහ පාරිභෝගික තොරතුරු සම්පූර්ණයෙන්ම පුද්ගලිකව පවතී. අපි ඔබේ පෞද්ගලිකත්වය සහ ව්‍යාපාර රහස්‍යභාවයට ප්‍රමුඛත්වය දෙමු.",
    q2_question: "මට Shopbook ව්‍යාපාර කිහිපයක් සඳහා භාවිතා කළ හැකිද?",
    q2_answer:
      "ඔව්, ඔබට ව්‍යාපාර කිහිපයක් සඳහා Shopbook භාවිතා කළ හැකිය. ඔබට ගිණුම් කිහිපයක් සාදා ඒවා වෙන වෙනම කළමනාකරණය කළ හැකිය.",
    q3_question: "යෙදුම නොමැති පාරිභෝගිකයින් සමඟ ඉන්වොයිසි බෙදා ගත හැකිද?",
    q3_answer:
      "ඔව්, ඔබට යෙදුම නොමැති පාරිභෝගිකයින් සමඟ ඉන්වොයිසි බෙදා ගත හැකිය. ඔබට ඉන්වොයිසි සබැඳිය ඔවුන් සමඟ බෙදා ගත හැකි අතර ඔවුන්ට තම දුරකථනයෙන් ඉන්වොයිසිය බැලිය හැකිය.",
    q4_question:
      "මගේ ජංගම දුරකථනය නැති වුවහොත්/හානි වුවහොත් මට නැවත මගේ දත්ත වෙත ප්‍රවේශ විය හැකිද?",
    q4_answer:
      "ඔව්, ඔබගේ ජංගම දුරකථනය නැති වුවහොත්/හානි වුවහොත් ඔබට නැවත ඔබගේ දත්ත වෙත ප්‍රවේශ විය හැක. ඔබට ඔබගේ විද්‍යුත් තැපෑල සහ මුරපදය සමඟ ඔබගේ ගිණුමට ලොග් විය හැකි අතර ඔබගේ දත්ත ඔබට ලබා ගත හැක.",
  },
  featuresSection: {
    main_heading_title: "ඔබේ ව්‍යාපාර ඉලක්ක Shopbook සමඟ ළඟා කරගන්න.",
    main_heading_subtitle:
      "Credit සහ Cash Invoice පහසුවෙන් සාදන්න",
    feature1_title: "ණය සහ මුදල් ඉන්වොයිසි පහසුයි",
    feature1_description:
      "ඔබගේ සියලුම ණය සහ මුදල් විකුණුම් ඉන්වොයිසි එක් තැනකින් කළමනාකරණය කරන්න.",
    feature2_title: "සියලුම පැරණි ඉන්වොයිසි ඕනෑම වේලාවක, එකම තැනකින් සොයා බලන්න",
    feature2_description: "ඕනෑම වේලාවක සියලුම පැරණි ඉන්වොයිසි බලා සොයන්න.",
    feature3_title: "Auto මතක් කිරීම් සමඟ වේගවත් ගෙවීම්",
    feature3_description:
      "ඉක්මන් ගෙවීම් එකතු කිරීම සඳහා SMS/WhatsApp හරහා පාරිභෝගිකයින්ට auto මතක් කිරීම් යවන්න",
    feature4_title: "ගෙවීම් නිරීක්ෂණය පහසුවෙන්",
    feature4_description:
      "ලැබුණු ගෙවීම් ඉක්මනින් සටහන් කර ඔබගේ ගිණුම් ක්ෂණිකව යාවත්කාලීන කරන්න.",
    feature5_title: "වාර්තා සාදන්න සහ බෙදාගන්න",
    feature5_description: "පහසුවෙන් වෘත්තීය විකුණුම් සහ ගෙවීම් වාර්තා සාදන්න.",
    feature6_title: "භාෂා 3කින් ලබාගත හැක",
    feature6_description:
      "Shopbook සිංහල, දෙමළ හෝ ඉංග්‍රීසි භාෂාවලින් භාවිතා කරන්න.",
    nested_cta_title: "ShopBook Auto ගෙවීම් මතක් කිරීම් යවා නොගෙවු ණය ඉක්මනින් එකතු කරගන්නේ  කෙසේදැයි බලන්න",
    nested_cta_description:
      "නොගෙවූ ණය අඩු කර ඉක්මනින් මුදල් එකතු කර ගැනීමට ShopBook වෘත්තීය ගෙවීම් මතක් කිරීම් පහසුවෙන් යවන්නේ කෙසේදැයි බලන්න.",
    nested_cta_button: "Demo වීඩියෝව බලන්න",
    phone_secure_alt: "ෂොප්‍බුක් ජංගම යෙදුම",
    money_hand_alt: "අතේ ඇති මුදල්",
  },
  learnsection: {
    main_title: "මිනිත්තු කිහිපයකින් Shopbook ප්‍රගුණ කරන්න",
    main_subtitle:
      "Shopbook වෙතින් උපරිම ප්‍රයෝජන ලබා ගැනීමට සහ ඉක්මනින් ගෙවීම් ලබා ගැනීමට ඔබට උපකාර වන සම්පූර්ණ නිබන්ධන සහ තිර ප්‍රවාහයන්.",
    video_tutorials_tab: "වීඩියෝ නිබන්ධන",
    screen_flows_tab: "තිර ප්‍රවාහයන්",
  },

  howItWorksSection: {
    video_tutorials_title: "වීඩියෝ නිබන්ධන",
    getting_started: "ආරම්භ කිරීම",
    create_share_invoices: "ඉන්වොයිස් සාදා බෙදාගන්න",
    collect_outstanding_credits: "පියවිය යුතු ණය එකතු කරන්න",
    set_auto_reminders: "ස්වයංක්‍රීය මතක් කිරීම් සකසන්න",
    view_due_customers: "ගෙවිය යුතු පාරිභෝගිකයින් සහ මුදල බලන්න",
    share_customer_reports: "පාරිභෝගික වාර්තා බෙදාගන්න",
    invoice_book: "ඉන්වොයිස් පොත",
    create_invoice_title: "Shopbook යෙදුම සමඟ ඉන්වොයිසියක් සාදා ගන්නේ කෙසේද",
    create_invoice_description: "ඉන්වොයිසියක් සෑදීමට පටන් ගන්න",
    how_to_create_invoice_title:
      "තත්පර කිහිපයකින් ඉන්වොයිසියක් සාදා ගන්නේ කෙසේද?",
    how_to_create_invoice_description:
      "තත්පර කිහිපයකින් ඉන්වොයිසියක් සාදා ගන්නේ කෙසේදැයි ඉගෙන ගන්න",
    login_process: "පිවිසුම් ක්‍රියාවලිය",
    how_to_login_title: "Shopbook යෙදුමට පිවිසෙන්නේ කෙසේද",
    how_to_login_description: "Shopbook යෙදුමට පිවිසෙන්නේ කෙසේද",
    how_to_login_existing_user_title:
      "ඔබේ යෙදුම මකා දැමුවහොත් හෝ ඔබ දැනට සිටින පරිශීලකයෙක් නම් Shopbook යෙදුමට පිවිසෙන්නේ කෙසේද",
    how_to_login_existing_user_description:
      "ඔබේ යෙදුම මකා දැමුවහොත් හෝ ඔබ දැනට සිටින පරිශීලකයෙක් නම් Shopbook යෙදුමට පිවිසෙන්නේ කෙසේද",
    items_adding: "භාණ්ඩ එකතු කිරීම",
    payment_tracking_reports_title: "ගෙවීම් නිරීක්ෂණ සහ වාර්තා",
    payment_tracking_reports_description:
      "ගෙවීම් නිරීක්ෂණය කර සවිස්තරාත්මක ව්‍යාපාර වාර්තා ජනනය කරන්න",
    add_items_invoice_title: "ඉන්වොයිසියක් සාදන විට භාණ්ඩ එකතු කරන්න",
    add_items_invoice_description:
      "ඉක්මනින් ගෙවීම් ලබා ගැනීමට WhatsApp සහ SMS මතක් කිරීම් යවන්න",
  },
  needMoreHelpSection: {
    main_title: "තවත් උදව් අවශ්‍යද?",
    main_subtitle: "ඔබේ සාර්ථකත්වයට සහාය වීමට අපි මෙහි සිටිමු",
    call_us_button: "අපිට කතා කරන්න",
    chat_with_us_button: "අප සමඟ කතා කරන්න",
    whatsapp_community_title: "WhatsApp ප්‍රජාව",
    join_now_button: "දැන්ම එකතු වන්න",
  },  footerSection: {
    description: "කුඩා හා මධ්‍ය පරිමාණ ව්‍යාපාර සඳහා වන සර්ව සම්පූර්ණ ව්‍යාපාර කළමනාකරණ විසඳුම. ඉන්වොයිසි සාදන්න, ගෙවීම් නිරීක්ෂණය කරන්න, සහ ඔබේ ව්‍යාපාරය වර්ධනය කරන්න.",
    get_in_touch_title: "අප අමතන්න",
    address_line1: "මිලේනියම් ටවර් (කෙලී ෆෙල්ඩර් ගොඩනැගිල්ල), 2 වන මහල, 345 ගාලු පාර, කොළඹ 00300, ශ්‍රී ලංකාව.",
    download_app_title: "යෙදුම බාගන්න",
    copyright_text: "© {{year}} Shopbook. සියලුම හිමිකම් ඇවිරිණි.",
    privacy_policy: "පෞද්ගලිකත්ව ප්‍රතිපත්තිය",
    made_in_text_part1: "",
    made_in_text_part2: "නිෂ්පාදිතයි",
    social_facebook_alt: "ෆේස්බුක්",
    social_instagram_alt: "Instagram",
    social_whatsapp_alt: "WhatsApp",
    social_youtube_alt: "YouTube",
    google_play_alt: "Google Play වෙතින් ලබා ගන්න",
    app_store_alt: "App Store වෙතින් බාගන්න",
  },contactus: {
    question1: "සහාය ඇත්තෙන්ම නොමිලේද?",
    answer1: "ඔව්! Shopbook භාවිතා කරන්නන් සඳහා සියලුම සහාය සම්පූර්ණයෙන්ම නොමිලේ. අපගේ භාවිතා කරන්නන්ට සාර්ථක වීමට උපකාර කිරීමට අපි විශ්වාස කරමු.",
    question2: "ඔබ කුමන භාෂා සඳහා සහාය දක්වන්නේද?",
    answer2: "විවිධ කලාපවල භාවිතා කරන්නන්ට උපකාර කිරීම සඳහා අපි බහු භාෂා සඳහා සහාය දෙමු.",
    question3: "ඔබ දුරකථන සහාය සපයනවාද?",
    answer3: "ඔව්, ඔබගේ ඕනෑම විමසීමක් සඳහා ඔබට සහාය වීමට අපගේ සහාය කණ්ඩායම දුරකථනයෙන් ලබා ගත හැකිය.",
    question4: "මගේ ගිණුම සැකසීමට ඔබට මට උදව් කළ හැකිද?",
    answer4: "අනිවාර්යයෙන්ම! ඔබේ Shopbook ගිණුම සැකසීමට අපගේ කණ්ඩායමට පියවරෙන් පියවර ඔබට මග පෙන්විය හැක.",
    image_alt: "Shopbook පාරිභෝගික සහාය කණ්ඩායම",
  }, supportSection: {
    main_title: "ඔබට සාර්ථක වීමට උපකාර කිරීමට අපි මෙහි සිටිමු",
    main_subtitle: "Shopbook වෙතින් උපරිම ප්‍රයෝජන ලබා ගැනීමට ඔබට උපකාර කිරීමට අපගේ කැපවූ සහාය කණ්ඩායම කැපවී සිටී.",
  }, headerSection: {
    logo_alt: "ෂොප්‍බුක් ලාංඡනය",
    nav_home: "මුල් පිටුව",
    nav_learn: "ඉගෙන ගන්න",
    nav_contact_us: "අප අමතන්න",
    download_button: "දැන් බාගන්න",
    lang_english: "English",
    lang_sinhala: "සිංහල",
    lang_tamil: "தமிழ்",
  },socialProofSection: {
    // Downloads
    downloads_subtitle: "15K+",
    downloads_title: "බාගතකිරීම්",
    downloads_image_alt: "පරිශීලක අවතාර",

    // Rating
    rating_subtitle: "4.5 ශ්‍රේණිගත කිරීම",
    rating_title: "App Store සහ Google Play",

    // Security
    security_main_text: "100%",
    security_subtitle: "ආරක්ෂිතයි සහ සුරක්ෂිතයි",
    security_title: "ඔබගේ දත්ත සුරක්ෂිතයි",

    // Languages
    languages_main_text: "3",
    languages_subtitle: "භාෂා",
    languages_title: "ඉංග්‍රීසි, සිංහල, දෙමළ",
  },
} as const;

export default TRANSLATIONS;
